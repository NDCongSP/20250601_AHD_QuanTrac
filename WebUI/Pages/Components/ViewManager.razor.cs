...
public partial class ViewManager<TItem>
{
    ...
    [Parameter] public RenderFragment<DetailViewContext<TItem>> DetailView { get; set; }
    ...
    public void ShowDetail(DetailViewData data, TItem item = default)
    {
        _showList = false;
        _showDetail = true;
        NeedsRefresh = false;
    ...
    // Xử lý các sự kiện
    private void HandleAddNew(DetailViewData data)
    {
        ShowDetail(data);
    }

    private void HandleEdit(DetailViewData data)
    {
        ShowDetail(data);
    }

    private void HandleView(DetailViewData data)
    {
        ShowDetail(data);
    }
