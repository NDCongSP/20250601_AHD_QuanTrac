...
{
    public partial class PopupBensuc : Form
    {
        public PopupBensuc()
        {
            InitializeComponent();
            ...
            chartBensuc.GetToolTipText += ChartMucNuoc_GetToolTipText;

        }
        private void DrawChart(List<dynamic> data)
        {
            chartBensuc.Series.Clear();
            chartBensuc.ChartAreas.Clear();

            var chartArea = new ChartArea("Bến súc");
            ...
            chartBensuc.ChartAreas.Add(chartArea);

            var series = new Series("Bến súc")
            {
            ...
            {
                series.Points.AddXY(item.Thời_gian, item.Mực_nước_bến_súc);
            }

            chartBensuc.Series.Add(series);

            // Tắt crosshair line — chỉ hiển thị tooltip
            chartBensuc.ChartAreas[0].CursorX.IsUserEnabled = false;
            chartBensuc.ChartAreas[0].CursorX.IsUserSelectionEnabled = false;
            chartBensuc.ChartAreas[0].CursorY.IsUserEnabled = false;
            chartBensuc.ChartAreas[0].CursorY.IsUserSelectionEnabled = false;

            // Bật tooltip cho từng điểm
            chartBensuc.Series[0].ToolTip = "⏰ #VALX{dd/MM/yyyy HH:mm}\n🌊 #VALY{F2} cm";
        }


        private void PopupBensuc_Load(object sender, EventArgs e)
        {
            LoadAndDisplayData();
        }
            ...
            try
            {
                ...
                var mappedData = rawData
                    .Select(x => new
                    {
                        ...
                        Mực_nước_bến_súc = x.Mực_nước_bến_súc
                    })
                    .ToList();
                ...
                if (dataGridView1.Columns.Contains("Mực_nước_bến_súc"))
                    dataGridView1.Columns["Mực_nước_bến_súc"].HeaderText = "Mực nước bến súc (cm)";
