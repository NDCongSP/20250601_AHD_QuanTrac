...
{
    public partial class PopupBinhnham : Form
    {
        public PopupBinhnham()
        {
            InitializeComponent();
            ...
            chartBinhnham.GetToolTipText += ChartMucNuoc_GetToolTipText;

        }
        private void DrawChart(List<dynamic> data)
        {
            chartBinhnham.Series.Clear();
            chartBinhnham.ChartAreas.Clear();

            var chartArea = new ChartArea("Bình nhâm");
            ...
            chartBinhnham.ChartAreas.Add(chartArea);

            var series = new Series("Bình nhâm")
            {
            ...
            {
                series.Points.AddXY(item.Thời_gian, item.Mực_nước_bình_nhâm);
            }

            chartBinhnham.Series.Add(series);

            // Tắt crosshair line — chỉ hiển thị tooltip
            chartBinhnham.ChartAreas[0].CursorX.IsUserEnabled = false;
            chartBinhnham.ChartAreas[0].CursorX.IsUserSelectionEnabled = false;
            chartBinhnham.ChartAreas[0].CursorY.IsUserEnabled = false;
            chartBinhnham.ChartAreas[0].CursorY.IsUserSelectionEnabled = false;

            // Bật tooltip cho từng điểm
            chartBinhnham.Series[0].ToolTip = "⏰ #VALX{dd/MM/yyyy HH:mm}\n🌊 #VALY{F2} cm";
        }


        private void PopupBinhnham_Load(object sender, EventArgs e)
        {
            LoadAndDisplayData();
        }
            ...
            try
            {
                ...
                var mappedData = rawData
                    .Select(x => new
                    {
                        ...
                        Mực_nước_bình_nhâm = x.Mực_nước_bình_nhâm,
                    })
                    .ToList();
                ...
                if (dataGridView1.Columns.Contains("Mực_nước_bình_nhâm"))
                    dataGridView1.Columns["Mực_nước_bình_nhâm"].HeaderText = "Mực nước bình nhâm (cm)";
