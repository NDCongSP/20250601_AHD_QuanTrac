...
{
    public partial class PopupTVDauTieng : Form
    {
        public PopupTVDauTieng()
        {
            InitializeComponent();
            ...
            chartTVdautieng.GetToolTipText += ChartMucNuoc_GetToolTipText;

        }
        private void DrawChart(List<dynamic> data)
        {
            chartTVdautieng.Series.Clear();
            chartTVdautieng.ChartAreas.Clear();

            var chartArea = new ChartArea("TV dầu tiếng");
            ...
            chartTVdautieng.ChartAreas.Add(chartArea);

            var series = new Series("TV dầu tiếng")
            {
            ...
            {
                series.Points.AddXY(item.Thời_gian, item.Mực_nước_TV_dầu_tiếng);
            }

            chartTVdautieng.Series.Add(series);

            // Tắt crosshair line — chỉ hiển thị tooltip
            chartTVdautieng.ChartAreas[0].CursorX.IsUserEnabled = false;
            chartTVdautieng.ChartAreas[0].CursorX.IsUserSelectionEnabled = false;
            chartTVdautieng.ChartAreas[0].CursorY.IsUserEnabled = false;
            chartTVdautieng.ChartAreas[0].CursorY.IsUserSelectionEnabled = false;

            // Bật tooltip cho từng điểm
            chartTVdautieng.Series[0].ToolTip = "⏰ #VALX{dd/MM/yyyy HH:mm}\n🌊 #VALY{F2} cm";
        }


        private void PopupTVDauTieng_Load(object sender, EventArgs e)
        {
            LoadAndDisplayData();
        }
            ...
            try
            {
                ...
                var rawData = dbContext.FT03s
                    ...
                    .Select(x => new
                    {
                        ...
                        Mực_nước_TV_dầu_tiếng = x.API_Fllow_DauTieng
                    })
                    .ToList();
                ...
                var mappedData = rawData
                    .Select(x => new
                    {
                        ...
                        Mực_nước_TV_dầu_tiếng = x.Mực_nước_TV_dầu_tiếng
                    })
                    .ToList();
                ...
                if (dataGridView1.Columns.Contains("Mực_nước_TV_dầu_tiếng"))
                    dataGridView1.Columns["Mực_nước_TV_dầu_tiếng"].HeaderText = "Mực nước TV dầu tiếng (cm)";
