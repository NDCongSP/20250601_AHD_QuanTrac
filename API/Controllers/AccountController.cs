...
namespace API.Controllers
{
    ...
    public class AccountController(IAccount account, ApiAuthenticationStateProvider _authStateProvider,HttpClient _httpClient) : ControllerBase
    {
        ...
        public async Task<ActionResult<GeneralResponse>> UpdateRoleDTOAsync(CreateRoleRequestDTO model)
        {
            if (model == null) return BadRequest("Model cannot be null");

            return Ok(await account.UpdateRoleDTOAsync(model));
        }
        ...
        {
            return Ok(await account.CheckPasswordAsync(email, password));
        }
    }
}
